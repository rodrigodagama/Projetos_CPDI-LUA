...
function Player.novo(x, y, hud)
    ...
    player.x = x
    player.y = y
    player.id = "player" -- utiliado na função de colisao local.
    ...
    -- ativando a funcção global .novo do script do teclado.
    scriptTeclado.novo(player)

    local function verificarColisao(self, event)
        if event.phase == "began" then
            -- quando o id do outrocorpo for "chao"
            if event.other.id == "chao" then
                -- Zeramos a variável numeroPulo do player para impedir o pulo duplo do chão.
                self.numeroPulo = 0
